...
public:
    ...
    void evaluateRelativeYawPersonVsRobot() {
        ...
        // RCLCPP_INFO(this->get_logger(), "ABS DEG: %f", abs_deg);

        const char* category = nullptr;
        if (abs_deg <= 40.0) {
            direction_person_ = -1; // BERLAWANAN ARAH
            category = "BERLAWANAN ARAH";
        } else if (abs_deg > 40.0 && abs_deg <= 140.0) {
        ...
        if (ttc > 0) {
            ...
            if (ttc < 3.0) {
                RCLCPP_WARN(this->get_logger(),
                    "TTC: %.2f s [BAHAYA] - Person approaching robot (< 3s)", ttc);
                return 1; // kode bahaya
            } else {
                RCLCPP_INFO(this->get_logger(),
                    "TTC: %.2f s [AMAN] - Person approaching robot (> 3s)", ttc);
                return 0; // kode aman
            }
        } else if (ttc == -2.0) {
            RCLCPP_INFO(this->get_logger(),
    ...
    bool isPersonMoving(const velocity& v) {
        return (v.vx > 0.3 || v.vy > 0.3 || v.vx < -0.3 || v.vy < -0.3);
    }
